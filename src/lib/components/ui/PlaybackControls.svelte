<script lang="ts">
    ...
    import {
        extract_title, fetch_file_data,
    } from "$lib/utils/fetch_jelly_data";

    interface MyProps {
        send_playlist_set?: (message: {
            playlist: TPlayListItem[];
            playlist_index: number;
        }) => void;
        send_video_set_playback_rate?: (message: {
            time: number;
            value: number;
        }) => void;
    }
    ...
    async function add_playlist_item(_event: Event) {
        ...
        const exists = temp_state.playlist.some(
            (item) => item.url === input_new_playlist_url,
        );
        if (!exists) {
            const index =
    ...
    function delete_playlist_item(_event: Event) {
        ...
        temp_state.playlist = temp_state.playlist.filter(
            (item) =>
                item.url === current_playing.url ||
                !select_playlist_items.includes(item.url),
        );
        const new_index = temp_state.playlist.findIndex(
            (item) => item.url === current_playing.url,
        );
    ...
    function set_playlist_index() {
        ...
        const target_index = temp_state.playlist.findIndex(
            (item) => item.url === selected_url,
        );
...
<div class="flex items-center space-x-2">
    ...
    />
    <button class="p-2 border hover:bg-blue-400" onclick={add_playlist_item}>
        Add to playlist
    </button>
</div>
<div class="flex items-center space-x-2">
